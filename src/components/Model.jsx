...
function Model() {
  // State to manage size and model details
  ...
  // Reference for camera controls and model groups
  ...
  // State to track the rotation angle of each model and set them back to 0
  const [smallRotation, setSmallRotation] = useState(0);
  const [largeRotation, setLargeRotation] = useState(0);

  const tl = gsap.timeline();

  // Effect to animate model views when the size changes
  useEffect(() => {
    if(size === 'large') {
